...
function InfoDetailProduct(props) {
    ...
    useEffect(() => {
        // Kiểm tra sản phẩm hiện tại đã có trong localStorage chưa
    ...
    let handleAddShopCart = () => {
        if (props.userId) {
            ...
            toast.error("Đăng nhập để thêm vào giỏ hàng", { position: "top-right" });
        }
    ...
    const handleBuyNow = () => {
        if (!props.userId) {
            toast.error("Đăng nhập để mua hàng", { position: "top-right" });
            return;
        }
    ...
    const handleConfirmBuyNow = () => {
        ...
        // 3. Tích sẵn vào ô sản phẩm vừa thêm (dựa vào productdetailsizeId)
        setTimeout(() => {
            history.push(`/shopcart?highlightProductDetailSizeId=${activeLinkId}`);
        }, 300); // Đợi redux cập nhật giỏ hàng
    };
    ...
    const handleToggleFavorite = () => {
        ...
        if (index === -1) {
            ...
            window.dispatchEvent(new Event('favoriteProductsUpdated'));
            setIsFavorite(true);
            toast.success('Đã thêm vào sản phẩm yêu thích!', { position: "top-right" });
        } else {
            // Xóa khỏi yêu thích
            ...
            window.dispatchEvent(new Event('favoriteProductsUpdated'));
            setIsFavorite(false);
            toast.info('Đã xóa khỏi danh sách yêu thích!', { position: "top-right" });
        }
    };
            ...
            <div className="col-lg-5 offset-lg-1">
                <div className="s_product_text">
                    ...
                    <div className="card_area">
                        ...
                        <a
                            className="main_btn"
                            style={{ marginLeft: 10, background: '#ff9800', color: '#fff', border: 'none' }}
                        ...
                        <Modal isOpen={showBuyNowModal} toggle={() => setShowBuyNowModal(false)} centered>
                            <ModalHeader toggle={() => setShowBuyNowModal(false)}>
                                Xác nhận mua hàng
                            </ModalHeader>
                            <ModalBody>
                                Bạn có chắc chắn muốn mua sản phẩm này ngay không?
                            </ModalBody>
                            <ModalFooter>
                                <Button color="primary" onClick={handleConfirmBuyNow}>Xác nhận</Button>
                                <Button color="secondary" onClick={() => setShowBuyNowModal(false)}>Hủy</Button>
                            </ModalFooter>
                        </Modal>
                    </div>
                </div>
